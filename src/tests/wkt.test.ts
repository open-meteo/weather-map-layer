...
/**
 * CAMS Europe, 0.1°, 700×420, stored north to south: the first grid point is
 * the north-western corner, so the BBOX "min" latitude is the northern edge.
 */
const CAMS_EUROPE_WKT = `GEOGCRS["WGS 84",
    DATUM["World Geodetic System 1984",
        ELLIPSOID["WGS 84",6378137,298.257223563]],
    CS[ellipsoidal,2],
        AXIS["latitude",north],
    ...
    USAGE[
        SCOPE["grid"],
        BBOX[71.95,-24.95,30.05,44.95]]]`;

/** Synthetic 1° grid crossing the antimeridian: 21 columns from 170°E to 170°W. */
const ANTIMERIDIAN_WKT = `GEOGCRS["WGS 84",
    DATUM["World Geodetic System 1984",
        ELLIPSOID["WGS 84",6378137,298.257223563]],
    CS[ellipsoidal,2],
        AXIS["latitude",north],
...
/** Grid spacing implied by inclusive first/last bounds: (last - first) / (n - 1). */
const spacing = (bounds: [number, number], n: number) => (bounds[1] - bounds[0]) / (n - 1);
...
describe('wktToGridData – regular grids', () => {
	test('ICON global: grid type, dimensions, first and last grid point', () => {
		const grid = wktToGridData(ICON_GLOBAL_WKT, 2880, 1441) as RegularGridFromBounds;
		expect(grid.type).toBe('regular');
		expect(grid.nx).toBe(2880);
		expect(grid.ny).toBe(1441);
		expect(grid.longitude).toEqual([0, 359.875]);
		expect(grid.latitude).toEqual([-90, 90]);
	});

	test('ICON global: spacing is 0.125° exactly (nx-1 intervals between first and last point)', () => {
		// The BBOX stores the positions of the first (lon=0) and last (lon=359.875)
		// grid points, so 359.875 / 2879 = 0.125 exactly because 2879 × 0.125 = 359.875.
		const grid = wktToGridData(ICON_GLOBAL_WKT, 2880, 1441) as RegularGridFromBounds;
		expect(spacing(grid.longitude, grid.nx)).toBe(0.125);
		expect(spacing(grid.latitude, grid.ny)).toBe(0.125);
	});

	test('GFS 0.25°: spacing is 0.25° exactly', () => {
		// 359.75 / (1440-1) = 0.25 exactly because 1439 × 0.25 = 359.75.
		const grid = wktToGridData(GFS_025_WKT, 1440, 721) as RegularGridFromBounds;
		expect(grid.type).toBe('regular');
		expect(spacing(grid.longitude, grid.nx)).toBe(0.25);
		expect(spacing(grid.latitude, grid.ny)).toBe(0.25);
	});
	...
	test('ICON and GFS global grids both have RegularGrid lonMax = 360° (correct for global wrap)', () => {
		...
		const iconData = wktToGridData(ICON_GLOBAL_WKT, 2880, 1441) as RegularGridFromBounds;
		const gfsData = wktToGridData(GFS_025_WKT, 1440, 721) as RegularGridFromBounds;
	...
	test('north-to-south grid keeps the first/last order, giving a negative row spacing', () => {
		const grid = wktToGridData(CAMS_EUROPE_WKT, 700, 420) as RegularGridFromBounds;
		expect(grid.latitude).toEqual([71.95, 30.05]);
		expect(spacing(grid.latitude, grid.ny)).toBeCloseTo(-0.1, 10);

		const [west, south, east, north] = new RegularGrid(grid).getBounds();
		expect(west).toBeCloseTo(-24.95, 6);
		expect(east).toBeCloseTo(45.05, 6);
		expect(south).toBeCloseTo(29.95, 6);
		expect(north).toBeCloseTo(71.95, 6);
	});

	test('grid crossing the antimeridian gets its last longitude lifted above the first', () => {
		const grid = wktToGridData(ANTIMERIDIAN_WKT, 21, 11) as RegularGridFromBounds;
		expect(grid.longitude).toEqual([170, 190]);
		expect(spacing(grid.longitude, grid.nx)).toBe(1);
	});
...
describe('wktToGridData – rotated lat/lon', () => {
	...
	test('KNMI: geographic bounds from BBOX', () => {
		const grid = wktToGridData(KNMI_ROTATED_WKT, 676, 564) as ProjectionGridFromBounds;
		expect(grid.latitude[0]).toBeCloseTo(39.740627, 6);
		expect(grid.latitude[1]).toBeCloseTo(62.619324, 6);
		expect(grid.longitude[0]).toBeCloseTo(-25.162262, 6);
		expect(grid.longitude[1]).toBeCloseTo(38.75702, 6);
	});

	test('KNMI: o_lat_p=35 → rotatedLat=-35 (the server writes o_lat_p = 90 - θ)', () => {
		const grid = wktToGridData(KNMI_ROTATED_WKT, 676, 564) as ProjectionGridFromBounds;
	...
	test('MeteoSwiss: o_lat_p=-43 (negative) → rotatedLat=43', () => {
		// Verifies that a negative o_lat_p flips to a positive rotatedLat.
	...
	test('MeteoSwiss: θ = 133° (negative o_lat_p gives θ > 90°)', () => {
		...
		// If the sign were wrong (not negated), we'd get θ = 47° instead of 133°.
		const grid = wktToGridData(METEOSWISS_ROTATED_WKT, 710, 640) as ProjectionGridFromBounds;
		const proj = new RotatedLatLonProjection(grid.projection as RotatedLatLonProjectionData);
...
describe('wktToGridData – Stereographic', () => {
	...
	test('geographic bounds from BBOX', () => {
		const grid = wktToGridData(STEREO_WKT, 1500, 1500) as ProjectionGridFromBounds;
		expect(grid.latitude[0]).toBe(56);
		expect(grid.latitude[1]).toBe(90);
		expect(grid.longitude[0]).toBe(-120);
		expect(grid.longitude[1]).toBe(120);
	});
...
describe('wktToGridData – Lambert Conformal Conic', () => {
	...
	test('geographic bounds from BBOX', () => {
		const grid = wktToGridData(LCC_WKT, 1906, 1606) as ProjectionGridFromBounds;
		expect(grid.latitude[0]).toBeCloseTo(39.671, 3);
		expect(grid.longitude[0]).toBeCloseTo(-25.421997, 6);
	});
...
describe('wktToGridData – Lambert Azimuthal Equal-Area', () => {
	...
	test('geographic bounds from BBOX', () => {
		const grid = wktToGridData(LAEA_WKT, 600, 600) as ProjectionGridFromBounds;
		expect(grid.latitude).toEqual([29, 73]);
		expect(grid.longitude).toEqual([-23, 45]);
	});
...
describe('wktToGridData – Gaussian grids', () => {
	test('O1280: returns gaussian type with correct latitude lines', () => {
		const grid = wktToGridData(GAUSSIAN_O1280_WKT, 6599680, 1) as GaussianGridData;
	...
	test('O1280: dimensions preserved', () => {
		const grid = wktToGridData(GAUSSIAN_O1280_WKT, 6599680, 1) as GaussianGridData;
		expect(grid.nx).toBe(6599680);
		expect(grid.ny).toBe(1);
	});

	test('O320: gaussianGridLatitudeLines = 320', () => {
		const grid = wktToGridData(GAUSSIAN_O320_WKT, 421120, 1) as GaussianGridData;
		expect(grid.type).toBe('gaussian');
		expect(grid.gaussianGridLatitudeLines).toBe(320);
	});

	test('throws for a regional (Area) reduced Gaussian grid', () => {
		// The server writes e.g. "Reduced Gaussian Grid O1280 Area (ECMWF)" for
		// European subsets; GaussianGrid only knows the global layout.
		const areaWkt = GAUSSIAN_O1280_WKT.replace(
			'Reduced Gaussian Grid O1280 (ECMWF)',
			'Reduced Gaussian Grid O1280 Area (ECMWF)'
		);
		expect(() => wktToGridData(areaWkt, 100, 1)).toThrow('Regional reduced Gaussian grids');
	});

	test('throws for a non-octahedral reduced Gaussian grid (N320)', () => {
		const n320Wkt = GAUSSIAN_O320_WKT.replace(
			'Reduced Gaussian Grid O320 (ECMWF)',
			'Reduced Gaussian Grid N320 (ECMWF)'
		);
		expect(() => wktToGridData(n320Wkt, 100, 1)).toThrow('Only octahedral');
	});
});
