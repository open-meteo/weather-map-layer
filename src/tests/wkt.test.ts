...
describe('wktToGridData – rotated lat/lon', () => {
	...
	test('KNMI: o_lat_p=35 → rotatedLat=-35 (negated for θ = 90 + rotatedLat)', () => {
		const grid = wktToGridData(KNMI_ROTATED_WKT, 676, 564) as ProjectionGridFromBounds;
	...
	test('MeteoSwiss: o_lat_p=-43 → rotatedLat=43 (negated for θ = 90 + rotatedLat)', () => {
		// Verifies that o_lat_p is negated to match the shared projection math.
