...
import { registerWarpTable } from '../grids/icon/icon-warp-tables';
import { readFileSync } from 'node:fs';
import { describe, expect, test } from 'vitest';

import type { IconGridData } from '../types';

// The table the browser fetches on demand (GridFactory.preload); registered
// directly here. Root division 2 has no table, so R2 grids are geometric.
const r3 = readFileSync(new URL('../grids/icon/icon-warp-r3.bin', import.meta.url));
registerWarpTable(3, r3.buffer.slice(r3.byteOffset, r3.byteOffset + r3.byteLength));
...
describe('GridFactory', () => {
	test('refuses to build an icon grid whose warp table is not loaded', () => {
		expect(() => GridFactory.create(smallGridData)).toThrow(/preload/);
	});

	test('builds and caches the global grid once its table is registered', () => {
		const grid = GridFactory.create(globalGridData);
		expect(grid).toBeInstanceOf(IconGrid);
		expect(GridFactory.create(globalGridData)).toBe(grid);
	});
});
