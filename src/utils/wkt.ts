...
export function wktToGridData(wkt: string, nx: number, ny: number): GridData {
	...
	if (parsed.type === 'GEOGCRS') {
		...
		if (derivingConversion) {
			...
			// Convert to the RotatedLatLonProjection format. o_lat_p is the
			// latitude of the rotated pole and lon_0 is the longitude rotation.
			// RotatedLatLonProjection computes θ = 90 + rotatedLat, matching the
			// Swift convention where the constructor latitude is the negated
			// o_lat_p (e.g. o_lat_p = 35 ⇒ latitude:-35 ⇒ θ = 55°).
			return {
				type: 'projectedFromBounds',
				projection: {
